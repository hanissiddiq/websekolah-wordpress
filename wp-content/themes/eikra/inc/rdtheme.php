...
if ( ! class_exists( 'RDTheme' ) ) {
	class RDTheme {
		...
		private function __construct() {
			$this->redux_init();
			add_action( 'after_setup_theme', [ $this, 'set_redux_option' ] );
			add_action( 'after_setup_theme', [ $this, 'set_redux_compability_options' ] );
		...
		public function redux_init() {
			add_action( 'admin_menu', [ $this, 'remove_redux_menu' ], 12 ); // Remove Redux Menu
			add_action( 'redux/loaded', [ $this, 'remove_redux_demo' ] ); // Removes the demo link and the notice
			add_filter( 'redux/eikra/aURL_filter', '__return_empty_string' ); // Remove Redux Ads

			// Flash permalink after options changed
			add_action( 'redux/options/eikra/saved', [ $this, 'flush_redux_saved' ], 10, 2 );
			add_action( 'redux/options/eikra/section/reset', [ $this, 'flush_redux_reset' ] );
			add_action( 'redux/options/eikra/reset', [ $this, 'flush_redux_reset' ] );
		}
		...
		public function set_redux_compability_options() {
			$new_options = [
				...
				'course_price'                         => true,
				'course_rating'                        => true,
				'course_archive_footer_visibility'     => true,
				'course_archive_instructor_visibility' => true,
				'course_archive_cat_filter'            => true,
				'course_archive_wishlist_visibility'   => true,
				'course_archive_enroll_visibility'     => true,
				'course_archive_review_visibility'     => true,
				'course_archive_price_visibility'      => true,
			];
		...
		public function remove_redux_demo() {
			if ( class_exists( 'ReduxFrameworkPlugin' ) ) {
				add_filter( 'plugin_row_meta', [ $this, 'redux_remove_extra_meta' ], 12, 2 );
		...
		public function flush_redux_saved( $saved_options, $changed_options ) {
			...
			$flush = false;
			$slugs = [ 'research_slug', 'event_slug', 'course_slug', 'instructor_slug' ];
