		...
		if ($schemaCat) {
			...
			switch ($schemaCat) {
				...
				case 'product':
					...
					if (! function_exists('rtrsp') && 'rtrs_affiliate' !== ( $metaData['post_type'] ?? '' ) ) {
						break;
					}
					...
					if (! empty($metaData['price'])) {
						...
						if( ! empty($metaData['handlingTimeMinimum']) ) {
							$shippingDeliveryTime["deliveryTime"]['handlingTime'] = [
						...
						if( ! empty($metaData['transitTimeMinimum']) ) {
							$shippingDeliveryTime["deliveryTime"]['transitTime'] = [
								"@type"    => "QuantitativeValue",
				...
				case 'web_page':
					...
					if ( ! empty( $metaData['webpage_url'] ) ) {
						$web_page_schema['@id'] = $helper->sanitizeOutPut( $metaData['webpage_url'], 'url' );
						$web_page_schema['url'] = $helper->sanitizeOutPut( $metaData['webpage_url'], 'url' );
					}
					...
					if ( ! empty( $metaData['dateModified'] ) ) {
						$web_page_schema['dateModified'] = $helper->sanitizeOutPut( $metaData['dateModified'] );
					}
					if ( ! empty( $metaData['dateModified'] ) ) {
						$web_page_schema['dateModified'] = $helper->sanitizeOutPut( $metaData['dateModified'] );
					}
					...
					if ($without_script) {
						$html = apply_filters('rtseo_snippet_web_page', $web_page_schema, $metaData);
					} else {
						$html .= $this->getJsonEncode(apply_filters('rtseo_snippet_web_page', $web_page_schema, $metaData));
					}
					break;
